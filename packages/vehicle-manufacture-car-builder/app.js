...
try {
  insurerConfig = Object.assign({}, config.get('insurer'));
} catch (err) {
...
if (process.env.INSURER_CONFIG ) {
  try {
    var insurerEnv = JSON.parse(process.env.INSURER_CONFIG);
    insurerConfig = Object.assign(insurerConfig, insurerEnv); // allow for them to only specify some fields
  } catch (err) {
    throw new Error('REST_SERVER_CONFIG invalid');
  }
}

if (!insurerConfig.hasOwnProperty('webSocketURL')) {
  throw new Error('The configuration for the insurer is invalid.')
}
