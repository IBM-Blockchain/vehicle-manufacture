      ...
      $scope.chain = response.data.map(function(transaction) {
        ...
        switch(type) {
          ...
          case 'UpdateOrderStatus': transactionSubmitter = transaction.eventsEmitted[0].order.vehicleDetails.make.replace('resource:org.acme.vehicle_network.Manufacturer#', '');
                                    ...
                                    break;
          case "CreatePolicy":      transactionSubmitter = transaction.eventsEmitted[0].insurer.replace('resource:org.acme.vehicle_network.Insurer#', '');
                                    displayTransaction = true;
                                    break;
          case "AddUsageEvent":     transactionSubmitter = "Vehicle ("+transaction.eventsEmitted[0].vehicle.replace('resource:org.acme.vehicle_network.Vehicle#', '')+")";
                                    displayTransaction = true;
                                    orderStatus = transaction.eventsEmitted[0].usageEvent.eventType;
                                    break;
        }
  ...
  function openWebSocket() {
    ...
    websocket.onmessage = function (event) {
      ...
      switch (message.$class) {
        case 'PlaceOrder':        caller = message.orderer.split('#')[1];
                                  break;
        case 'UpdateOrderStatus': caller = message.order.vehicleDetails.make.split('#')[1];
                                  status = message.orderStatus;
                                  break;
        case "CreatePolicy":      caller = message.insurer.split('#')[1];
                                  break;
        case "AddUsageEvent":     caller = 'Vehicle ('+message.vehicle.split('#')[1]+')';
                                  status = message.usageEvent.eventType;
                                  break;
      }

      $scope.addBlock(message.eventId.split('#')[0], message.$class, caller, status);
      $scope.$apply();
    }
  }
