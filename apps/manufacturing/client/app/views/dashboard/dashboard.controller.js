...
.controller('DashboardCtrl', ['$scope', '$http', '$interval', function ($scope, $http, $interval) {
  ...
  const baseContractUrl = baseUrl + '/org.acme.vehicle_network.vehicles';
  ...
  $http.get(baseContractUrl + '/orders', {headers: {'Authorization': `Basic ${btoa('system:systempw')}`}})
  ...
  async function getOrderHistory(orderId) {
    const resp = await new Promise((resolve, reject) => {
      return $http.get(`${baseContractUrl}/orders/${orderId}/history`, {headers: {Authorization: `Basic ${btoa('system:systempw')}`}})
        .then((res, err) => {
          if (err) {
            return reject(err);
          }
  ...
  function setupPlaceOrderListener() {
    const orderUpdates = new EventSource(baseContractUrl + '/orders/events/placed');
  ...
  var generateTelematicId = async function() {
    const telematicDevice = await new Promise((resolve, reject) => {
      $http.post(baseUrl + '/wallet/devices/telematic', {}, {headers: {'Authorization': 'Basic ' + btoa('system:systempw')}}).then((response, err) => {
        if (err) {
          reject(err);
          return;
        }
    ...
    await registerTelematicDevice(telematicDevice.name);
    return telematicDevice.name + '@Arium';
  }

  var registerTelematicDevice = async (id) => {
    return new Promise((resolve, reject) => {
      $http.post(baseUrl + '/org.acme.vehicle_network.participants/devices/telematic/register', { name: id }, {headers: {'Authorization': 'Basic ' + btoa('system:systempw')}}).then((response, err) => {
          if (err) {
            reject(err);
            return;
          }
  ...
  var updateOrderStatus = async function(orderId, count) {
    ...
    if (count === 2) {
      ...
      status.telematicId = await generateTelematicId();
    }

    const timestamp = Date.now();

    $http.put(`${baseContractUrl}/orders/${orderId}/status`, status, {headers: {'Authorization': 'Basic ' + btoa('system:systempw')}}).then(function(response, err) {
      if(err) {
